  ...
  describe 'As a admin user' do
    ...
    describe "see all orders" do
      before :each do
        ...
        @mike = Merchant.create(name: "gary's Print Shop", address: '123 Paper Rd.', city: 'Denver', state: 'CO', zip: 80203, enabled: true)
        @meg = Merchant.create(name: "luis's Bike Shop", address: '123 Bike Rd.', city: 'Denver', state: 'CO', zip: 80203, enabled: true)
...
it "disable and enable merchat and items" do
    ...
    mike = Merchant.create(name: "Mike's Print Shop", address: '123 Paper Rd.', city: 'Denver', state: 'CO', zip: 80203, enabled: true)
    santi = Merchant.create(name: "Meg's Bike Shop", address: '123 Bike Rd.', city: 'Denver', state: 'CO', zip: 80203, enabled: false)
    tire = santi.items.create(name: "Gatorskins", description: "They'll never pop!", price: 100, image: "https://www.rei.com/media/4e1f5b05-27ef-4267-bb9a-14e35935f218?size=784x588", inventory: 12)
    paper = mike.items.create(name: "Lined Paper", description: "Great for writing on!", price: 20, image: "https://cdn.vertex42.com/WordTemplates/images/printable-lined-paper-wide-ruled.png", inventory: 3)
    pencil = mike.items.create(name: "Yellow Pencil", description: "You can write on paper with it!", price: 2, image: "https://images-na.ssl-images-amazon.com/images/I/31BlVr01izL._SX425_.jpg", inventory: 100)
    order_1 = Order.create(name: user.name, address: user.address, city: user.city, state: user.state, zip: user.zip, user_id: user.id, status: 1)
    order_2 = Order.create(name: user.name, address: user.address, city: user.city, state: user.state, zip: user.zip, user_id: user.id, status: 2)
    ...
    expect(page).to have_link("Disable")
      ...
      within "#merch-#{mike.id}" do
        click_on "Disable"
        expect(current_path).to eq("/admin/merchants")
        expect(page).to_not have_link("Disable")
      end
      within "#merch-#{santi.id}" do
        click_on "Enable"
        expect(current_path).to eq("/admin/merchants")
        expect(page).to have_link("Disable")
      end
      ...
      within "#merch-#{santi.id}" do
        ...
        expect(current_path).to eq(admin_merchant_show_path(santi))
